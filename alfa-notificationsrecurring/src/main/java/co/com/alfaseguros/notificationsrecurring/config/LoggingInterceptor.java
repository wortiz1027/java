...
public class LoggingInterceptor implements ClientHttpRequestInterceptor {
    ...
    private void logRequest(HttpRequest request, byte[] body) {
        if (log.isDebugEnabled()) {
            ...
            if(!request.getURI().toString().contains("alfa-digital") && !request.getURI().toString().contains("notifications/api/v1/emails"))
            	log.debug("Request body: {}", json(new String(body, StandardCharsets.UTF_8)));
